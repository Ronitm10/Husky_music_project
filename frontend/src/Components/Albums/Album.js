...
const Album = () => {
  ...
  const likeSong = async (trackId) => {
    ...
    const userRes = await axios.get(
      `http://localhost:4000/api/users/find/${userToken.user.id}`
    );
    ...
    axios
      .post(
        `http://localhost:4000/api/users/updateLikes/${userToken.user.id}`,
        {
          likedTracks: payload,
        }
      )
  ...
  useEffect(() => {
    axios
      .get(`http://localhost:4000/api/albums/find/${albumId}`)
      .then((res) => {
